...
computational implementation is adapted to the parameterisation and
...
(Gelman et al., 2020) so that SAE modelling becomes systematic: prior
predictive checks, MCMC convergence diagnostics, posterior predictive
checks, leave-one-out cross-validation, Bayesian model comparison and
averaging, prior sensitivity analysis, and design-consistent
benchmarking are all part of the standard pipeline.

> **Scope.** This package focuses on **area-level** SAE models.
...
Assess MCMC convergence (replaces the legacy
...
Evaluate model fit and compare models (replaces the legacy
...
Validate prior assumptions before fitting (replaces the legacy
...
Generate predictions with proper uncertainty quantification, including
for unsampled areas (replaces the legacy
...
Two custom families are shipped (Loglogistic and Shifted Loglogistic)
with full support for `loo()`, `posterior_predict()`, and
`posterior_epred()`:
...
Register your own custom family via
...
The package implements area-level hierarchical Bayesian SAE following
the methodological tradition of Rao and Molina (2015), adapted to the
...
- **Principled Bayesian workflow** (Gelman et al., 2020) integrated
  throughout
...
This package was developed at **Politeknik Statistika STIS, Jakarta**
by:

- **Achmad Syahrul Choir** ([ORCID
  0000-0001-7088-0646](https://orcid.org/0000-0001-7088-0646)) — author,
  maintainer
...
- Use the interactive Shiny app
  ([`run_sae_app()`](https://madsyair.github.io/hbsaems/reference/run_sae_app.md))
  for guided model building
- Reference website (when published):
