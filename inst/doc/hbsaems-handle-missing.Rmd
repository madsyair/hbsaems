...
Missingness is the rule rather than the exception in small area
work.  Three patterns are most common:
...
| `"model"` | Joint Bayesian imputation via `brms::mi()` | Treats missing values as parameters |
...
for these areas can still be obtained via `sae_predict()` with their
auxiliary values.

## Strategy 2: multiple imputation (mice)

For covariates that are missing in some areas but observable in
principle from a census or register, multiple imputation with `mice`
is the workhorse:
...
When the missingness is suspected MAR or MNAR and you have a good
model for the missing variable, joint Bayesian imputation via
`brms::mi()` is more principled:
...
This fits a **multivariate** brms model where missing values of `y`
and `x1` are sampled alongside the regression coefficients.  Posterior
uncertainty in the imputations is automatically propagated.

Not every family supports `mi()` -- check
`spec$supports_mi` in the model registry (`?list_hbsae_models`).
...
* **Small fraction missing (< 5%) and likely MCAR** -- deletion is
  fine.
* **Larger fraction or non-MCAR** -- prefer multiple imputation or
  model-based.
* **Missing in response only and you have administrative auxiliaries
  for those areas** -- prefer deletion + post-fit prediction via
  `sae_predict()` (this is more transparent than imputing the
  response).
