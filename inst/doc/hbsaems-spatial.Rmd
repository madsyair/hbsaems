...
The simplest specification: pass `sre`, `sre_type = "car"`, and `M`.
...
* Use `sre_type = "car"` (default ICAR) as the SAE workhorse spatial
  prior.
* Use `car_type = "bym2"` together with `re = ~(1 | sre)` for the
  modern Riebler et al.\ parameterisation.
* SAR (`sre_type = "sar"`) is appropriate when the spatial mechanism
