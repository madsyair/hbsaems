...
test_that("check_spatial_weight rejects non-matrix", {
  expect_error(check_spatial_weight(1:5, spatial_model = "car"),
               "must be a matrix")
})

test_that("check_spatial_weight rejects non-numeric", {
  expect_error(check_spatial_weight(matrix("a", 2, 2), spatial_model = "car"),
               "must be numeric")
})
...
test_that("non-square matrix flagged", {
  M <- matrix(0, 3, 4)
  res <- check_spatial_weight(M, spatial_model = "car", verbose = FALSE)
...
test_that("asymmetric matrix is only WARNING for SAR", {
  ...
  res <- check_spatial_weight(M, spatial_model = "sar", verbose = FALSE)
...
test_that("isolated areas flagged as warning", {
  ...
  res <- check_spatial_weight(M2, spatial_model = "car", verbose = FALSE)
...
test_that("compatible TRUE only when no fatal issues", {
  M <- .valid_car_matrix()
  expect_true(check_spatial_weight(M, spatial_model = "car",
                                     verbose = FALSE)$compatible)

  M2 <- M; M2[1, 2] <- 2  # asymmetric -> fatal for CAR
  expect_false(check_spatial_weight(M2, spatial_model = "car",
                                      verbose = FALSE)$compatible)
})
