...
The **intercept** uses a prior of `normal(22, 5)`. This reflects a central belief around 22 (near the average home value) with moderate uncertainty, covering the plausible range of housing prices, which typically span from 5 to 50.

The **regression coefficients** are assigned the following priors:

-   **crim**, **indus**, **dis**, and **rad**:\
    Each has a `normal(0, 2)` prior. These predictors may have moderate effects on home prices, but we maintain a neutral stance (mean = 0) with enough variance (SD = 2) to allow the model to learn from the data. This is a relatively **weakly informative prior**, meaning it assumes little but still avoids extremely implausible values.

-   **rm** (average number of rooms per dwelling):\
    A more informative prior of `normal(3, 1)` is used. This reflects the expectation that each additional room has a strong positive effect on housing prices, and the smaller SD focuses the prior near that belief.

-   **tax** (property tax rate per \$10,000):\
    A conservative prior of `normal(0, 0.1)` is used. Since the `tax` variable has a large range, this tight prior implies that small changes in tax are unlikely to have a major effect on home values, effectively shrinking the coefficient toward zero.
