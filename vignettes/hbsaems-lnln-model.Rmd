---
title: "Lognormal-Lognormal SAE Model"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Lognormal-Lognormal SAE Model}
...
agricultural yield, firm revenue, length of hospital stay.  Modelling
the log of the response with a normal hierarchy is a workhorse choice
in SAE (Slud & Maiti 2006; You & Chapman 2006).
...
`sigma` appears in the "Further Distributional Parameters" block --
it is being sampled.
...
original (back-transformed) scale by `sae_predict()`:
...
* `hbm_lnln()` covers positive right-skewed survey responses with a
  lognormal likelihood.
* Pin $\sigma$ via `sampling_variance = "psi_i"` to get the Fay-Herriot
...
* You, Y., & Chapman, B. (2006).  Small area estimation using area
  level models and estimated sampling variances.  *Survey Methodology*
  32(1), 97-103.
