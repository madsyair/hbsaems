...
We will use `data_betalogitnorm` to simulate the `hbm_betalogitnorm` function. The `data_betalogitnorm` is a simulation data created specifically to demonstrate the implementation of _Hierarchical Bayesian Small Area Estimation_ (HB SAE) with Beta distribution. This data is suitable for testing Beta regression models with a hierarchical structure between areas. This data is also equipped with variables that apply spatial effects.
...
- The `n` variable indicates the number of sample units in each area or region used in the survey
- `deff` states the design effect of the survey. 
