...
> vignette are **realistic illustrations** – representative of what you
> would see on a real run, but not produced by the chunks above. To
> reproduce the exact numbers, copy the code into an interactive R
> session.
...
`hbsaems` is a thin, opinionated layer over
[`brms`](https://paulbuerkner.com/brms/) that gives SAE practitioners:
...
- the **universal entry point**
  ...
  fully custom models including custom brms response distributions;
...
- you need a likelihood that the wrappers don’t expose (e.g. `lognormal`
  with logit-link `mu`, or a custom registered family);
...
Use [`hbm()`](https://madsyair.github.io/hbsaems/reference/hbm.md) when:

- you have an arbitrarily complex
  [`brms::bf()`](https://paulbuerkner.com/brms/reference/brmsformula.html)
  formula;
- you need multivariate response with
