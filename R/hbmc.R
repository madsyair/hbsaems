...
#' Kallioinen, N., Paananen, T., Buerkner, PC. et al. Detecting and diagnosing prior and likelihood sensitivity with power-scaling.
#' Statistical Computing, 34, 57 (2024).
  ...
  if (length(all_brms_models) > 1 && length(comparison_metrics) > 0) {
    ...
    for (i in 2:length(all_brms_models)) {
      ...
      safe_data_subset_i <- if(!is.null(data_for_loo_waic_i) && !is.null(vars_in_formula_i)) {
        ...
        if("phi_fixed" %in% names(data_for_loo_waic_i)) {
          vars_present_i <- c(vars_present_i, "phi_fixed")
        }
