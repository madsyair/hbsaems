...
- **Multiple Distribution Families**: Support for Gaussian, Beta, Binomial/Logit-Normal, and Lognormal distributions
- **Spatial Modeling**: Conditional Autoregressive (CAR) and Simultaneous Autoregressive (SAR) spatial random effects  
...
- `hbm_beta()`: For proportion data (0 < y < 1) using Beta distribution
- `hbm_betalogitnorm()`: For Beta distribution with logit-normal structure
- `hbm_binlogitnorm()`: For binomial data with logit-normal model
- `hbm_logitnormal()`: Alternative logit-normal implementation
- `hbm_lnln()`: For lognormal-lognormal models
- `hbm_lognormal()`: For lognormal distribution
