...
Loads the contents of `inst/stan/<name>.stan` from the installed hbsaems
(or from the local source tree when running tests). This is a thin
wrapper around [`readLines()`](https://rdrr.io/r/base/readLines.html)
that resolves the package path once and validates that the file exists.
...
- name:
  ...
  Character. The distribution name – must match a `<name>.stan` file
  shipped under `inst/stan/`.
